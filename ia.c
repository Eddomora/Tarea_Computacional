...
void orient_edge_by_mode(int u, int v, int mode, int *from, int *to) {
    ...
    switch (mode) {
        ...
        case 3: // paridad de (lo+hi): si par lo->hi; si impar hi->lo
            if (((lo + hi) % 2) == 0) { *from = lo; *to = hi; }
            else { *from = hi; *to = lo; }
            break;
        case 4: // paridad de lo: si lo par lo->hi; si impar hi->lo
            if ((lo % 2) == 0) { *from = lo; *to = hi; }
            else { *from = hi; *to = lo; }
            break;
        default: *from = lo; *to = hi; break;
    }
}
...
void dijkstra(int G[MAX][MAX], int n, int start_node, int end_node) {
    ...
    for (int step = 0; step < n; ++step) {
        ...
        visited[u] = 1;
        if (u == end_node) break;     // corte temprano: ya alcanzamos destino

        for (int v = 0; v < n; ++v) {
            if (!visited[v] && G[u][v] != 0) { // arista u->v
...
int main(void) {
    ...
    char planar_edges[][2] = {
        {'a','b'},{'a','h'},{'b','h'},{'b','d'},{'b','c'},{'c','d'},
        {'d','i'},{'d','f'},{'d','e'},{'e','f'},{'f','i'},{'f','h'},{'f','g'},{'g','h'}
    };
    for (int i = 0; i < 14; ++i) {
    ...
    char eulerian_edges[][2] = {
        {'a','b'},{'a','h'},{'b','c'},{'b','j'},{'b','h'},{'c','j'},{'c','d'},{'c','e'},
        {'d','j'},{'d','i'},{'d','e'},{'e','j'},{'e','i'},{'e','f'},{'e','g'},{'f','i'},
        {'f','h'},{'f','g'},{'g','i'},{'g','h'},{'h','i'},{'h','j'},{'i','j'}
    };
    for (int i = 0; i < 23; ++i) {
    ...
    if (scanf("%d", &directed) != 1) return 1;
    if (directed) {
        printf("Elija orientacion (1..4): ");
        if (scanf("%d", &orient_mode) != 1) return 1;
        if (orient_mode < 1 || orient_mode > 4) orient_mode = 1;
